                            ...
                            <div class="panel-body">
                                <form method="post" class="form-horizontal">
                                    <div class="form-group user_type agent" style="display: block;">
                                        <label class="col-sm-2 control-label" for="">Select Agent : </label>
                                        <div class="col-sm-10">
                                            <div class="row">
                                                <div class="col-sm-4">
                        ...
                        <table id="example">
                            ...
                            <tbody>
                                ...
                                for ($i = 0; $i < count($inquiries); $i++) {
                                    echo "<tr>";
                                    echo "<td data-th='id.' hidden><div>" . $inquiries[$i]->id . "</div></td>";
                                    echo "<td data-th='Reference No'><div>" . $inquiries[$i]->property_ref_no . "</div></td>";
                                    echo "<td data-th='Inquiry No'><div>" . $inquiries[$i]->incquiry_ref_no . "</div></td>";
                                    if ($inquiries[$i]->aquired == 'sale') {
                                        echo "<td data-th='Property Status'> <div>Sale</div> </td>";
                                    } else if ($inquiries[$i]->aquired == 'rent') {
                                        echo "<td data-th='Property Status'><div> Rent </div></td>";
                                    } else if ($inquiries[$i]->aquired == 'both') {
                                    ...
                                    echo "<td data-th='Created by'><div>" . $inquiries[$i]->u_fname . ' ' . $inquiries[$i]->u_lname . "</div></td>";
                                    echo "<td data-th='Date Created'><div>" . date("d-M-Y", strtotime($inquiries[$i]->created_date)) . "</div></td>";
                                    ?>
                                <input type="hidden" id="<?php echo trim($inquiries[$i]->id); ?>" value="<?php echo trim($inquiries[$i]->id); ?>" name="<?php echo trim($inquiries[$i]->id); ?>">
                                <td data-th="Status">
                                    <?php /* <a href="javascript:void(0)" data-id="<?php echo trim($inquiries[$i]->id); ?>" data-status="1" class="btn btn-default btn-small">Action</a> */ ?>
                                    <div class="btn-group">
                                        <a href="javascript:void(0)" data-ref="<?php echo trim($inquiries[$i]->incquiry_ref_no); ?>" data-id="<?php echo trim($inquiries[$i]->id); ?>" data-status="1" class="btn btn-default btn-small action-agent-status">Confirm</a>
                                        <a href="javascript:void(0)" data-ref="<?php echo trim($inquiries[$i]->incquiry_ref_no); ?>" data-id="<?php echo trim($inquiries[$i]->id); ?>" data-status="2" class="btn btn-default btn-small action-agent-status">Reschedule</a>
                                        <a href="javascript:void(0)" data-ref="<?php echo trim($inquiries[$i]->incquiry_ref_no); ?>" data-id="<?php echo trim($inquiries[$i]->id); ?>" data-status="3" class="btn btn-default btn-small action-agent-status">Cancel</a>
                                    </div>
                                </td>
                                <td data-th="Actions">
                                    <div>
                                        <a data-toggle="modal" data-target="#myModal" onclick="setInquiryId(<?php echo $inquiries[$i]->id; ?>)" class="btn btn-default btn-xs action-btn" rel="tooltip" title="View Inquiry"><i class="fa fa-eye"></i></a> 
                                        <!-- <a href="#popup2" class="btn btn-info btn-xs" onclick="setInquiryId(<?php echo $inquiries[$i]->id; ?>)">View Inquiry</a>  -->
                                        <?php if ($this->session->userdata('logged_in_super_user')) { ?>
                                            <a href="delete_inquiry/<?php echo $inquiries[$i]->id; ?>" onclick="return confirm('Are you sure want to delete this record?');" class="btn btn-danger btn-xs action-btn" rel="tooltip" title="Delete"><i class="fa fa-trash"></i></a>

                        <!--  &nbsp;<a href="delete_inquiry/<?php echo $inquiries[$i]->id; ?>" onclick="return confirm('Are you sure want to delete this record?');" class="btn btn-danger btn-xs">Delete</a> -->
                                        <?php } ?>
                                    </div>
                                </td>
                                <?php
                                //echo "<td>";
                                //echo '<a class="btn btn-default btn-small" href="#popup2" title="View Inquiry" onclick="setInquiryId(' . $inquiries[$i]->id . ')"><i class="icon-zoom-in"></i></a>';
                                // if ($this->session->userdata('logged_in_super_user')) {
                                //   echo anchor('inquiry/delete_inquiry/' . $inquiries[$i]->id, '<i class="icon-trash"></i>', array('onclick' => "return confirm('Are you sure you want to delete?')",'title'=>'Delete Inquiry','class'=>'btn btn-default btn-small'));
                                // }
                                //echo "</td>";
                                echo "</tr>";
                            }
