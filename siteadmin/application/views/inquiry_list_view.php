        ...
        <div class="main">
            ...
            <h1 class="page-header">Inquiry List
                <?php if ($this->session->userdata('logged_in_super_user')) { ?>
                    <a><button data-toggle="modal" data-target="#myModal3" class="btn btn-sm btn-info pull-right" type="button">Import Inquiries</button></a>
                <?php } ?>
            </h1>
            <div class="row">
                    ...
                    <div class="col-md-3">
                        <span>View Inquiry By :  &nbsp;&nbsp;
                            <select name="view_inc"  id="view_inc" class="form-control" style="width:200px" onchange="view_inquiry(this.value);">
                                <option value="" <?php if (empty($calendar['view'])) {
                    echo "selected";
                } ?> >All</option>
                                <option value="rent" <?php if (!empty($calendar['view']) && $calendar['view'] == "rent") {
                    echo "selected";
                } ?> >Rent inquiry</option>
                                <option value="sale" <?php if (!empty($calendar['view']) && $calendar['view'] == "sale") {
                    echo "selected";
                } ?>>Sale inquiry</option>
                            </select>
                        </span> 
...
<?php ?>
                                <option value="" <?php if (empty($calendar['view_client'])) {
    echo "selected";
} ?>>All</option>
<?php foreach ($all_client as $key => $value) { ?>
                                    <option value="<?php echo $value->id; ?>" <?php if (!empty($calendar['view_client']) && $calendar['view_client'] == $value->id) {
        echo "selected";
...
<?php } ?>
                ...
                <div class="col-md-5">
                    <span style="display: inline-block;float: left">Search :
                        <input type="text" value="<?php if (isset($calendar) && !empty($calendar['inc_search'])) {
    echo $calendar['inc_search'];
} ?>" id="inc_search" class="form-control" style="width:200px" />
                    </span>
                    <input type="button" style="margin-top: 22px;margin-left:10px" id="inc_search_btn" class="btn btn-sm btn-info" value="Go">
                </div>
                <div class="clearfix sep"></div>
                <div class="clearfix sep"></div>
            </div>
...
<form id="calendar-filter-form" name="calendar-filter-form" method="post">
    ...
    <input type="hidden" id="calendar_inc_search" name="calendar[inc_search]" value="<?php echo (isset($calendar) && isset($calendar['inc_search']) && !empty($calendar['inc_search'])) ? $calendar['inc_search'] : ""; ?>" />
</form>
...
<script type="text/javascript">
                                                        ...
                                                        $(document).ready(function () {
                                                            ...
                                                            $("#inc_search_btn").on("click", function () {
                                                                var search_val = $("#inc_search").val().trim();

                                                                $("#calendar_inc_search").val(search_val);
                                                                $("#calendar-filter-form").submit();
                                                            });
